...
for label = 1:4 
    ...
    for Fields = 1:ID.File(end) 
        ...
        for n = 1:length(nuclei) 
            ...
            limit = 15;
            % one of the channels of overlap plus singles
            IndexPart1 = nan(length(OrderPairs), limit); 
            % the other channel of overlap
            IndexPart2 = nan(length(OrderPairs), limit); 
            % overlap distance
            overlapDist = nan(length(OrderPairs), limit); 
            ...
            for FlO = 1:length(OrderPairs) 
                ...
                if isempty(channelPicked{2}) % single-color probes
                    ...
                    [A, B] = find(Dist < ThresholdDetection); 
...
%% Functions to run after analysis
% laminaDISTeigen(Coordinates, Lamina)
